...
export default function PortfolioDetailHeader({ data }: PortfolioDetailHeaderProps) {
  if (!data) return null;

  const BadgeIcon = data.badgeIcon ? (iconMap[data.badgeIcon as keyof typeof iconMap] || Sprout) : Sprout;

  const getMetaIcon = (iconName: string) => {
    const IconComponent = iconMap[iconName as keyof typeof iconMap] || iconMap.default;
    return <IconComponent className="w-5 h-5 text-[#2c7a3f] shrink-0 stroke-[2]" aria-hidden="true" />;
  };
