...
export default function ServiceFeaturesApproach({ data }: ServiceFeaturesApproachProps) {
  if (!data) return null;
    ...
    <section className="font-sans px-4 sm:px-6 lg:px-8 max-w-[1348px] mx-auto mt-10 sm:mt-14 lg:mt-16">
      ...
      <div className="rounded-3xl bg-[#f4f8f4] border border-[#e2ede2] p-4 sm:p-6 lg:p-8">
        ...
        <div className="text-center flex items-center justify-center gap-2 sm:gap-4 mb-6 sm:mb-8">
          {data.topBannerIcon && (
            <span className="text-[#2c7a3f] text-lg sm:text-xl">{data.topBannerIcon}</span>
          )}
          ...
          {data.approach?.steps && data.approach.steps.length > 0 && (
            <div className="mt-4 space-y-4">
              {data.approach.steps.map((step, idx) => (
                <div key={idx} className="flex items-start gap-3.5 p-2 rounded-xl hover:bg-gray-50 transition-colors">
