                      ...
                      <ul className="flex flex-col gap-0.5">
                        {link.children?.map((child) => (
                          <li key={child.label}>
                            <Link
                              href={child.href}
                              className="block py-2 px-3.5 rounded-lg text-xs xl:text-sm font-medium text-gray-700 hover:text-[#0c4d1e] hover:bg-[#0c4d1e]/5 transition-all whitespace-nowrap"
                            >
                              {child.label}
                            </Link>
                          </li>
        ...
        <div className="lg:hidden border-t border-gray-100 bg-white py-4 px-4 sm:px-6 shadow-lg">
          <nav className="flex flex-col gap-2">
                    ...
                    <div className="ml-3 mt-1.5 pl-2 border-l-2 border-[#2c7a3f] flex flex-col gap-1 py-1">
                      {link.children?.map((child) => (
                        <Link
                          ...
                          href={child.href}
                          onClick={() => setIsOpen(false)}
                          className="block text-sm font-medium text-gray-700 hover:text-[#0c4d1e] py-1.5 px-2 rounded-md hover:bg-gray-100/80"
                        >
                          {child.label}
                        </Link>
                      ))}
            ...
            <div className="flex flex-col gap-3 mt-2">
              ...
              {[
                { ...data.actions.donate, icon: Heart, style: "sm:hidden border-[#2c7a3f] text-[#2c7a3f] hover:bg-[#2c7a3f] hover:text-white" },
                { ...data.actions.volunteer, icon: Users, style: "md:hidden bg-[#0c4d1e] text-white hover:bg-[#083815]" }
              ].map((btn) => {
